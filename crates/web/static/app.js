...
async function K(id) {
  ...
  try {
    ...
    if (pv) {
      if (c.status === 'running') {
        pv.innerHTML = '<img src="/api/cameras/' + encodeURIComponent(c.id) + '/live" alt="Live preview" style="width:100%;height:100%;object-fit:contain;border-radius:var(--r);background:#000" onerror="kE(\'' + encodeURIComponent(c.id) + '\')" />';
      } else {
